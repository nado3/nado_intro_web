...
// Keep the public teachers page aligned with the current selectable roster.
(() => {
  const canonical = document.querySelector('link[rel="canonical"]')?.href || '';
  if (!canonical.endsWith('/teachers.html')) return;

  const grid = document.querySelector('.teachers-grid');
  if (!grid) return;

  const planByName = {
    'Vitalina Skardova': '이코노미',
    'Amelia Lee': '스탠다드',
    'Ej': '스탠다드',
    'Oscar': '프리미엄',
    'Victoria Yuguy': '스탠다드',
  ...
  [...grid.querySelectorAll('.teacher-card')].forEach(card => {
    ...
    const tags = card.querySelector('.teacher-tags');
    if (plan && tags && !tags.querySelector('.teacher-plan-tag')) {
      const badge = document.createElement('span');
      badge.className = 'tag teacher-plan-tag';
      badge.textContent = plan;
      tags.prepend(badge);
    }
  });

  const sectionLabel = document.querySelector('.teachers-section .section-label');
  if (sectionLabel) sectionLabel.textContent = '대표 선생님';

  const sectionDesc = document.querySelector('.teachers-section .section-desc');
  if (sectionDesc) {
    sectionDesc.innerHTML = '현재 활동 중인 대표 선생님을 소개합니다.<br>실제 신청 시 플랜, 지역, 시간대에 따라 지금 선택 가능한 선생님을 확인할 수 있습니다.';
  }

  const cta = document.querySelector('#more-teachers');
  if (cta) {
    setText(cta, '.cta-title', '현재 가능한 선생님을 확인해보세요');
    setText(cta, '.cta-text', '신청에서 플랜, 지역, 시간대를 입력하면 조건에 맞는 선생님을 직접 확인하고 선택할 수 있어요.');
    setText(cta, '.btn-primary', '선생님 확인하고 신청하기');
  }
})();
